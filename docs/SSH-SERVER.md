...
| 能力 | 状态 | 证据 |
|---|---|---|
| 密钥登录 | ✅ | `ssh vangogh` → uid 10297 |
| su 提权 root | ✅ | `su -c id` → uid 0 |
| 脱离 adb | ✅ | 断开 adb 后实测通过 |
| **开机自启** | ✅ | 重启后日志：`11:16:48 看门狗启动` → `11:16:49 拉起 sshd` |
| 手动启动脚本 | ✅ | 目标机 `~/ssh-on` |
| sshd 挂掉后自愈 | ⚠️ **未验证** | 看门狗逻辑已修正，但测试时无法可靠地把 kill 命令送进设备（受链路抖动影响） |

### 开机自启的三个必要条件（缺一不可）

Magisk 确实会执行 `service.d`（日志里有 `service.d: exec [99-sshd.sh]`），但要真正跑起来还需要：

1. **入口必须 `setsid` 脱离进程组**再后台化。直接 `( ... ) &` 的话，脚本返回时
   后台子 shell 会被一起回收 —— 日志文件都不会产生。
2. **检测必须用 `pidof sshd`**，不能用 `ps -A | grep "[s]shd"`：
   后者的模式会匹配到看门狗脚本自己的名字（`sshd-watchdog.sh`），
   于是永远认为 sshd 还活着，从不拉起。
3. 看门狗拆成独立文件 `/data/adb/sshd-watchdog.sh`，便于单独测试与排查。

## 为什么"从会话手动拉看门狗"不可靠

即使 `setsid`，从 SSH / adb shell 会话启动的进程仍在同一 cgroup，
会话结束时会被一并回收。所以：

- **开机路径**（Magisk 启动）→ 进程不属于任何会话 → 能长期存活 ✅
- **手动路径** → 请用装在 Termux 里的 `~/ssh-on`（它带 `termux-wake-lock`），
  不要去手动拉看门狗
